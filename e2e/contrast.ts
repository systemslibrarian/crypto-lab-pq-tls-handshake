 ...
 *    an unknown backdrop, or a `color-mix()`. This page is made of them.
 *    `<body>` paints THREE stacked `radial-gradient`/`linear-gradient` layers
 *    over its `--bg0` backstop, so every element that does not paint its own
 *    opaque background is text on a gradient. On top of that, `.exhibit` is
 *    `rgba(10, 17, 28, 0.84)`, `.panel` is `rgba(19, 30, 46, 0.85)`, `.arrow` is
 *    `rgba(255, 215, 0, 0.08)`, `.banner` is `rgba(0, 255, 136, 0.08)`, the
 *    three `.group/.x25519/.mlkem` highlight spans in the wire dump are
 *    `rgba(…, 0.23–0.25)`, `.hkdf-box` is
 *    `color-mix(in oklab, var(--hybrid) 6%, var(--code-bg))`, the hero aside is
 *    `color-mix(in oklab, var(--client) 6%, transparent)`, and both MTU fills
 *    are `color-mix(in oklab, …, transparent)`. axe files all of them under
 *    `incomplete`, which means a violations-only gate measured the contrast of
 *    almost nothing this page renders.
 ...
 *    at `.85` and, the one that matters, `.arrow` at `.35`. Both arrows carry
 *    real words ("ClientHello", "ServerHello") and both ship faded, because
 *    `.arrow.active` — which restores `opacity: 1` — is applied only once the
 *    handshake has been stepped past them. The arrival state of this page is
 *    therefore two labels at 35% opacity, and no gate had ever measured them.
 *
 ...
 * the walk as a pair rather than fading the foreground alone. `.arrow` is
 * exactly that shape: a translucent yellow pill and its label fading together.
 *
 ...
 * not behind it. On this page that is `.mtu-fill`, which is
 * `position: absolute` inside its track. So an ancestor's own paint is applied
 ...
 *    boxes are `.wire-block` (`max-height: 280px; overflow: auto`, which the
 *    hex dump overflows the moment it is shown) and `.mtu-track`
 *    (`overflow: hidden`). Content scrolled past those is not dimmed or partly
 *    drawn — it is absent from the frame, its rect falls outside every
 *    ancestor's box, and a walk that measured it would find nothing behind it
 *    and fall through to WHITE, inventing a failure for every hex byte below
 *    the fold of the dump.
 *
 ...
 *    draws no SVG figures of its own — every diagram here is HTML and CSS grid,
 *    and the only SVG on the page is the shared header's two glyph-free
 *    `aria-hidden` marks. The branch is kept because the alternative is a helper
 *    that silently mismeasures the first labelled diagram anyone adds.
 *
 ...
 * carry meaning: `.arrow.reverse::before { content: "< " }` and
 * `.arrow:not(.reverse)::after { content: " >" }`, which are the only thing
 * saying which DIRECTION each wire message travels in.
 *
 * SECOND, the `box-shadow` on `.arrow.active` and on `.btn:hover`/`.chip:hover`
 * (`0 0 0 1px rgba(255,215,0,0.2) inset`). A shadow is not a border and not a
 * background, so neither oracle composites it. It is decorative reinforcement
 * in both cases — the hover rule also sets `border-color: var(--wire)`, and the
 * active arrow also goes to full opacity — and it is hand-measured rather than
 * assumed.
 */
 ...
 * uses. It can afford to, because this page has no short-lived styling to miss:
 * every render is a synchronous full `innerHTML` rewrite of `#app`, and the two
 * animations (`.shell`'s `fade-in`, `.arrow.active`'s infinite `pulse`) are
 * both reduced to a 0.01ms duration by the stylesheet's reduced-motion block,
 * which this gate asserts is in effect. Auto-play advances on a 1200ms
 * interval, and the drive stops it before scanning rather than racing it.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * that is `.wire-block` (`overflow: auto` under a 280px cap, which the hex
     * dump overflows as soon as it is shown) and `.mtu-track`
     * (`overflow: hidden`, which is what keeps each bar's fill inside its
     * track). This lab has no `.sr-only` idiom, but the guard for it is handled
     * separately below and kept.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // would apply the moment this lab draws its MTU bars or its key-schedule
      // flow in SVG rather than in CSS grid.
     ...
     *
     * What this lab hides was enumerated by grepping `aria-hidden` across
     * `index.html` and `src/`, and it is a closed set of nine: the shared
     * header's two SVG marks, the two `.dot` legend swatches (each sitting
     * immediately before the words it colour-codes), the `||` concatenation
     * operator and the three `.flow-down` arrow blocks in the key-schedule
     * figure (each of which is restated in the prose above it), and the MTU
     * visual's three `.mtu-track` / `.mtu-boundary` rows.
     *
     * The MTU rows are the one place this boundary has a real cost, and it is
     * stated rather than waved away: their `.mtu-bar-label`s carry the two byte
     * counts. Those exact numbers appear in `.metric-note` directly beneath, in
     * `--ink` on the `.exhibit` surface, and in the "Total hello bytes" metric
     * above — all of which this walk DOES measure — so the ink is covered even
     * though those particular nodes are skipped.
     *
     * Nothing else on this page hides a VALUE: no derived secret, no capsule
     * hex, no wire byte and no metric is inside an `aria-hidden` subtree. That
     * was checked rather than assumed, because it is the shared blind spot
     * where both oracles stop looking.
     */
     ...
     * how this lab builds its DOM: one enormous `innerHTML` template literal
     * rebuilt on every render. Interpolating a string ARRAY into one makes JS
     * join it with commas, leaving a run of "," text nodes inside whatever
